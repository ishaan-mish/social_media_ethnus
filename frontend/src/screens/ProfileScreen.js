...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const res = await fetch(`https://social-media-ethnus.onrender.com/api/users/${userId}`);
  ...
  const handleFollow = async () => {
    ...
    try {
      const res = await fetch(`https://social-media-ethnus.onrender.com/api/users/${currentUser._id}/${endpoint}`, {
        method: 'POST',
...
/>
      ...
      <div style={{ marginTop: "15px" }}>
        <button
          onClick={async () => {
            try {
              const res = await fetch(`https://social-media-ethnus.onrender.com/api/users/${user._id}/update`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
